...
class CombinerRNN(nn.Module):
    ...
    def forward(self, word1, word2, hidden, combine=False):
        ...
        if not combine:
            ...
            word2_relu = self.W2(word2).clamp(min=0)
            word_concat = torch.cat([word1_relu, word2_relu], dim=1).view(1, 1, -1)
            # qn = torch.norm(word_concat, p=2, dim=1).detach()
            # word_concat = word_concat.div(qn.expand_as(word_concat))
            output, hidden = self.gru(word_concat, hidden)
        return output, hidden
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, word, hidden, split=False):
        ...
        if not split:
            ...
            # extended_embedding = self.W3(embedding)
            ...
            # cluster1 = self.W1(cluster1)
            # cluster2 = self.W2(cluster2)

            return cluster1, cluster2

    def embeddingToWord(self, embedding, hidden):
        output = F.relu(embedding).view(1, 1, -1)
        word, hidden = self.gru(output, hidden)
        word = F.log_softmax(self.out(word[0]))
        return word, hidden
...
def train(input_variable, embedding, detector, combiner, decoder, detector_optimizer, combiner_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for i in range(input_length-1):
        ...
        if current_sentence.size()[1] != 2:
            ...
            for j in range(current_sentence.size()[1] - 1):
                ...
                # loss += (1 - criterion(word_in_front, word_behind)) * similarity_weight
            ...
            combined_index = scores.index(min(scores))
            loss += scores[combined_index]
            ...
            # for i in range(input_length):
            #     loss += criterion(current_representation[:, i, :], input_variable_embedded[:, i, :])/input_length
            ...
            for i in path[::-1]:
                ...
                if i == 0:
                    ...
                    current_representation = torch.cat([current_representation[:, :i, :], word_in_front, word_behind,
                                                        current_representation[:, i+1:, :]], 1)
            output_hidden = decoder.initHidden()
            for i in range(input_length):
                loss += criterion(current_representation[:, i, :], input_variable_embedded[:, i, :])/input_length
                word, output_hidden = decoder.embeddingToWord(current_representation[:, i, :], output_hidden)
                try:
                    similarity = 1 - spatial.distance.cosine(current_representation[:, i, :].data.cpu().numpy(), current_representation[:, i+1, :].data.cpu().numpy())
                    print(similarity)
                except IndexError:
                    pass

                # loss += 0.05*criterion2(word, input_variable[i])
                topv, topi = word.data.topk(1)
                ni = topi[0][0]
                output_sentence.append(all_class.index2word[ni])
    ...
    return loss.data[0], path, encoded_sent, output_sentence
...
def trainIters(detector, combiner, decoder, n_iters, print_every=100, plot_every=100, learning_rate=0.01):
    ...
    for i in range(n_iters):
        ...
        for iter in range(len(all_sent)):
            ...
            loss, path, encoded_sentence, output_sentence = train(input_variable, embedding, detector, combiner,
            ...
            if iter % print_every == 0:
                ...
                print(current_sentence, path, output_sentence)
...
decoder1 = DecoderRNN(hidden_size, all_class.num_words)
...
trainIters(detector1, combiner1, decoder1, 1, print_every=50)
