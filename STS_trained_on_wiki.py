...
sents = list()
count = 0
with open('/home/dw1215/PycharmProjects/Embedding_wiki/data/wiki_text_sentsplit_tokenized_split_aa', 'r') as file:
    print('reading lines')
...
for line in lines:
    ...
    for pp in p:
        pps = pp.split(' ')
        if 20 >= len(pps) >= 3 and pp[-1].isalpha():
            sents.append(pp)
            count += 1
...
#
#     sentences = [[normalize_string(s) for s in l.split('\t')][0] for l in lines]
#
#     input_ = Lang(lang1)
#
...
sent_wiki = [normalize_string(l) for l in sents]
wiki_class = Lang('wiki')
for sentence in sent_wiki:
    wiki_class.addSentence(sentence)

# embedding_matrix = np.zeros((wiki_class.num_words, hidden_size))
# for word, i in wiki_class.word2index.items():
...
def trainIters(data_set, train_sentences, detector, combiner, decoder, n_iters, print_every=100, plot_every=100, learning_rate=0.05, save_embedding=False):
    start = time.time()
    plot_losses = []
    ...
    # print('saving model')
    # torch.save(combiner.state_dict, 'trained_model/combiner_test.pt')
    # print('model saved')
    ...
    embedding_matrix = np.zeros((data_set.num_words, hidden_size))
    for word, i in data_set.word2index.items():
        embedding_vector = embeddings_index.get(word)
        if embedding_vector is not None:
            # words not found in embedding index will be all-zeros.
            embedding_matrix[i] = embedding_vector

    embedding = nn.Embedding(data_set.num_words, hidden_size)
    embedding.weight.data.copy_(torch.from_numpy(embedding_matrix))
    embedding = embedding.cuda()

    for i in range(n_iters):
        if save_embedding:
            file = open('/home/dw1215/PycharmProjects/first_pytorch/data/encoded_upd_every_node_wiki{}'.format(i), 'w')

        training_sentences = [variableFromSentence(data_set, train_sentences[k]) for k in
                              range(len(train_sentences))]

        for iter in range(len(train_sentences)):
            ...
            for w in input_variable.data.cpu().numpy():
                current_sentence.append(data_set.index2word[int(w)])
            ...
            if iter % print_every == 0:
                ...
                print_loss_total = 0
                print('%s (%d %d%%) %.4f' % (timeSince(start, iter / (n_iters * len(train_sentences))),
                                             iter + i * len(train_sentences), (iter + i * len(train_sentences))/(n_iters * len(train_sentences)) * 100, print_loss_avg))
        ...
        torch.save(combiner.state_dict, 'trained_model/combiner{}.pt'.format(i))
        torch.save(detector.state_dict, 'trained_model/detector{}.pt'.format(i))
        torch.save(decoder.state_dict, 'trained_model/decoder{}.pt'.format(i))
...
trainIters(wiki_class, sent_wiki, detector1, combiner1, decoder1, 1, print_every=50, save_embedding=False)
trainIters(all_class, all_sent, detector1, combiner1, decoder1, 1, print_every=50, save_embedding=True)
