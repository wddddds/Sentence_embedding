...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_outputs2 = Variable(torch.zeros(max_length, encoder.hidden_size))
    encoder_outputs2 = encoder_outputs2.cuda() if use_cuda else encoder_outputs

    for ei in range(input_length):
        encoder_output2, encoder_hidden2 = encoder(
            input_variable[ei], encoder_hidden2)
        encoder_outputs2[ei] = encoder_output2[0][0]
    loss += input_variable.size()[0]*mse_loss(encoder_hidden, encoder_hidden2)
