...
for l in data_read[1:]:
    # l = line[:-5].replace('"', '')
    # print(line[:-5])
    if len(l[-9].split(' ')) > 20 or len(l[-8].split(' ')) > 20:
        pass
    else:
        sent1.append(l[-9])
        sent2.append(l[-8])
...
print(sent2[:5])
print(labels[:5])
print('total number of training samples: ', count)
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    for i in range(n_iters):
        ...
        file = open('/home/dw1215/PycharmProjects/first_pytorch/data/encoded_SNLI_normalized+Glove{}'.format(i), 'w')
        ...
        for iter in range(len(all_sent)):
            ...
            if iter % print_every == 0:
                ...
                print_loss_total = 0
                print('%s (%d %d%%) %.4f' % (timeSince(start, iter / len(all_sent)),
                                             iter, (iter + i * len(all_sent)) / (n_iters * len(all_sent)) * 100, print_loss_avg))
...
trainIters(encoder1, attn_decoder1, 20, print_every=100)
